...
export const is_mandatory_input_given = function (input) {
  ...
  for (const element of input) {
    ...
    if (
      element.id === "ac" &&
      element.value !== null &&
      [775000000, 775000001].includes(element.value)
    ) {
      ac = true;
    }
    if (
      element.id === "pr" &&
      element.value !== null &&
      [775000000, 775000001, 775000002].includes(element.value)
    ) {
      pr = true;
    }
    if (
...
export const calculate_base_score = function (
  Impact,
  exploitability,
  scope_value
) {
...
export const calculate_MISS = function (cr, mc, ir, mi, ar, ma, c, i, a) {
  ...
  const first_value =
    1 -
    (1 - cr * internal_mc) * (1 - ir * internal_mi) * (1 - ar * internal_ma);
...
export const calculate_ModifiedImpact = function (
  MISS,
...
export const calculate_ModifiedExploitability = function (
  ...
  const result =
    8.22 * intenal_mav * internal_mac * internal_mpr * internal_mui;
...
export const calculate_Overall_CVSS_vector = function (input, data) {
  ...
  const ModifiedImpact = calculate_ModifiedImpact(
    MISS,
    modified_scope_value,
    scope_value
  );
